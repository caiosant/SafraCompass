class ApplicationController < ActionController::API
    ...
    def user_process_token
        if request.headers['UserAuthorization'].present?
          begin
            jwt_payload = JWT.decode(request.headers['UserAuthorization'].split(' ')[0].remove('"'), Rails.application.secrets.secret_key_base).first
    ...
    def authenticate_advisor!(options = {})
    ...
    def advisor_process_token
      if request.headers['AdvisorAuthorization'].present?
        begin
          jwt_payload = JWT.decode(request.headers['AdvisorAuthorization'].split(' ')[0].remove('"'), Rails.application.secrets.secret_key_base).first
